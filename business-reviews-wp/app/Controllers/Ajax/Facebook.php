...
class Facebook {
	...
	function fb_page_access_token() {
		$user_id      = ( ! empty( $_REQUEST['user_id'] ) ) ? sanitize_text_field( wp_unslash( $_REQUEST['user_id'] ) ) : ''; //phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$access_token = ( ! empty( $_REQUEST['access_token'] ) ) ? sanitize_text_field( wp_unslash( $_REQUEST['access_token'] ) ) : ''; //phpcs:ignore WordPress.Security.NonceVerification.Recommended
