...
class Functions {
	...
	public static function get_nonce() {
		...
		return isset( $_REQUEST[ rtbr()->getNonceId() ] ) ? sanitize_text_field( wp_unslash( $_REQUEST[ rtbr()->getNonceId() ] ) ) : null;
	}

	public static function sanitize_css_dimension( $value ) {
	...
	public static function sanitize_css_weight( $value ) {
		$allowed = array( 'normal', 'bold', 'bolder', 'lighter', '100', '200', '300', '400', '500', '600', '700', '800', '900' );
		return in_array( $value, $allowed, true ) ? $value : '';
	}

	public static function sanitize_css_align( $value ) {
	 ...
	 * @return bool|mixed|void
	 */
	function fieldGenerator( $fields = [], $multi = false ) {
		$html = null;
	...
	public static function shortcode_style( $sc_id, $style_tag = false ) {
		...
		$sc_meta['floating_badge_pos'] = isset( $_REQUEST['floating_badge_pos'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['floating_badge_pos'] ) ) : get_post_meta( $sc_id, 'floating_badge_pos', true );
		$sc_meta['width']              = isset( $_REQUEST['width'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['width'] ) ) : get_post_meta( $sc_id, 'width', true );

		$sc_meta['badge_bg']             = isset( $_REQUEST['badge_bg'] ) ? sanitize_hex_color( wp_unslash( $_REQUEST['badge_bg'] ) ) : get_post_meta( $sc_id, 'badge_bg', true );
		...
		$sc_meta['btn_text_color']       = isset( $_REQUEST['btn_text_color'] ) ? sanitize_hex_color( wp_unslash( $_REQUEST['btn_text_color'] ) ) : get_post_meta( $sc_id, 'btn_text_color', true );
		$sc_meta['btn_bg_color']         = isset( $_REQUEST['btn_bg_color'] ) ? sanitize_hex_color( wp_unslash( $_REQUEST['btn_bg_color'] ) ) : get_post_meta( $sc_id, 'btn_bg_color', true );
		$sc_meta['btn_text_hover_color'] = isset( $_REQUEST['btn_text_hover_color'] ) ? sanitize_hex_color( wp_unslash( $_REQUEST['btn_text_hover_color'] ) ) : get_post_meta( $sc_id, 'btn_text_hover_color', true );
		$sc_meta['btn_bg_hover_color']   = isset( $_REQUEST['btn_bg_hover_color'] ) ? sanitize_hex_color( wp_unslash( $_REQUEST['btn_bg_hover_color'] ) ) : get_post_meta( $sc_id, 'btn_bg_hover_color', true );
		...
		if ( $value = self::sanitize_css_dimension( $sc_meta['width'] ) ) {
			$css .= "#review-list-content-{$sc_id} { ";
			$css .= 'width:' . $value . ';';
			$css .= '}';
		}

		$title = ( ! empty( $sc_meta['business_title'] ) ? $sc_meta['business_title'] : [] );
		if ( ! empty( $title ) ) {
			...
			if ( $title_color ) {
				$css .= 'color:' . self::sanitize_css_color( $title_color ) . ';';
			}
			if ( $title_size ) {
				$css .= 'font-size:' . $title_size . 'px;';
			}
			if ( $title_weight ) {
				$css .= 'font-weight:' . self::sanitize_css_weight( $title_weight ) . ';';
			}
			if ( $title_alignment ) {
				$css .= 'text-align:' . self::sanitize_css_align( $title_alignment ) . ';';
			}
			$css .= '}';
		}

		$title = ( ! empty( $sc_meta['business_title_hover'] ) ? $sc_meta['business_title_hover'] : [] );
		if ( ! empty( $title ) ) {
			$title_color     = ( ! empty( $title['color'] ) ? $title['color'] : null );
			$title_size      = ( ! empty( $title['size'] ) ? absint( $title['size'] ) : null );
			$title_weight    = ( ! empty( $title['weight'] ) ? $title['weight'] : null );
		...
		$title = ( ! empty( $sc_meta['author_name'] ) ? $sc_meta['author_name'] : [] );
		if ( ! empty( $title ) ) {
			$title_color     = ( ! empty( $title['color'] ) ? $title['color'] : null );
			$title_size      = ( ! empty( $title['size'] ) ? absint( $title['size'] ) : null );
			$title_weight    = ( ! empty( $title['weight'] ) ? $title['weight'] : null );
		...
		$title = ( ! empty( $sc_meta['author_name_hover'] ) ? $sc_meta['author_name_hover'] : [] );
		if ( ! empty( $title ) ) {
			$title_color     = ( ! empty( $title['color'] ) ? $title['color'] : null );
			$title_size      = ( ! empty( $title['size'] ) ? absint( $title['size'] ) : null );
			$title_weight    = ( ! empty( $title['weight'] ) ? $title['weight'] : null );
		...
		$title = ( ! empty( $sc_meta['review_text'] ) ? $sc_meta['review_text'] : [] );
		if ( ! empty( $title ) ) {
			$title_color     = ( ! empty( $title['color'] ) ? $title['color'] : null );
			$title_size      = ( ! empty( $title['size'] ) ? absint( $title['size'] ) : null );
			$title_weight    = ( ! empty( $title['weight'] ) ? $title['weight'] : null );
		...
		$title = ( ! empty( $sc_meta['rating_text'] ) ? $sc_meta['rating_text'] : [] );
		if ( ! empty( $title ) ) {
			$title_color     = ( ! empty( $title['color'] ) ? $title['color'] : null );
			$title_size      = ( ! empty( $title['size'] ) ? absint( $title['size'] ) : null );
			$title_weight    = ( ! empty( $title['weight'] ) ? $title['weight'] : null );
		...
		$title = ( ! empty( $sc_meta['time_ago_text'] ) ? $sc_meta['time_ago_text'] : [] );
		if ( ! empty( $title ) ) {
			$title_color     = ( ! empty( $title['color'] ) ? $title['color'] : null );
			$title_size      = ( ! empty( $title['size'] ) ? absint( $title['size'] ) : null );
			$title_weight    = ( ! empty( $title['weight'] ) ? $title['weight'] : null );
		...
		$title = ( ! empty( $sc_meta['total_review_text'] ) ? $sc_meta['total_review_text'] : [] );
		if ( ! empty( $title ) ) {
			$title_color     = ( ! empty( $title['color'] ) ? $title['color'] : null );
			$title_size      = ( ! empty( $title['size'] ) ? absint( $title['size'] ) : null );
			$title_weight    = ( ! empty( $title['weight'] ) ? $title['weight'] : null );
		...
		$title = ( ! empty( $sc_meta['powered_by_text'] ) ? $sc_meta['powered_by_text'] : [] );
		if ( ! empty( $title ) ) {
			$title_color     = ( ! empty( $title['color'] ) ? $title['color'] : null );
			$title_size      = ( ! empty( $title['size'] ) ? absint( $title['size'] ) : null );
			$title_weight    = ( ! empty( $title['weight'] ) ? $title['weight'] : null );
		...
		if ( $value = self::sanitize_css_dimension( $sc_meta['btn_border_radius'] ) ) {
			$css .= "#review-list-content-{$sc_id} .rtbr-pagination.rt-loadmore-btn a { ";
