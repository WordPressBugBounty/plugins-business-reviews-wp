...
<div class="rtbr-pagination rt-loadmore-btn">
	...
	if ( $sc_meta['pagination'] && $sc_meta['layout'] != 'isotope-one' ) :
		$pagi_num = $sc_meta['reviews_per_page']; // phpcs:disable
		if ( count( $review_data ) <= $pagi_num ) {
			return;
		}
		if ( ! $pagi_num ) {
			$pagi_num = 10; // phpcs:disable
		}
		?>
	...
	if ( $sc_meta['see_all_reviews'] == '' && $business_info->getBusinessType() != 'multiple' ) {
		echo '<a rel="nofollow" class="rtbr-see-all-review" ' . wp_kses_post( $sc_meta['open_link_blank'] ) . ' href="' . esc_url( $business_info->getAllReviewUrl() ) . '">' . esc_html__( 'See All Reviews', 'business-reviews-wp' ) . '</a>';
	}
